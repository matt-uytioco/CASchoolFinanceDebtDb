...
d3.csv('../csvUpload/public/finance_data.csv').then(data => {
...
// fetch('/api/export')
//     .then(() => {
//         d3.csv("./exported_data.csv").then(data => {
//             // Filter relevant attributes (excluding leaid and other non-relevant columns)
//             const relevantAttributes = Object.keys(data[0]).filter(key => 
//                 !["year", "lea_id", "leaid", "phone", "lea_name", "urban_centric_locale", "fiscal year", 
//                 "debt_shortterm_outstand_end_fy","payments_charter_schools","name of school district",""].includes(key)
...
//                         .html(`Missing data for ${selectedAttribute}`);
//                 })
...
//     const attributes = Object.keys(parsedData.get(currentDistrict)[0]).filter(attr => 
//         !["year", "lea_id", "leaid"].includes(attr)
//     );
...
//         option.value = attr;
//         option.textContent = attr;
//         attributesList.appendChild(option);
//     });
// }